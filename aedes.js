...
function emitPacket (packet, done) {
  ...
  if (!chaosEng.active) {
    ...
    chaosEng.emitPacket(packet, defaultBehaviour)
  }
}
...
function callPublished (_, done) {
  //- Execute published callback and emit publish event for listeners (such as the logger the cli attaches, for example)
...
Aedes.prototype.subscribe = function (topic, func, done, client = null) {
  if (chaosEng.active) {
    chaosEng.interceptSubscribe(topic, func, done, client)
  } else {
...
Aedes.prototype.unsubscribe = function (topic, func, done) {
  if (chaosEng.active) {
    chaosEng.interceptUnsubscribe(topic, func, done)
  } else {

...
function Client (broker, conn, req) {
  // console.log('------CLIENT INIT, client connected------')
  ...
  function nextBatch (err) {
    ...
    if (that._parsingBatch <= 0) {
      ...
      // - Reading data from the conn socket
      const buf = client.conn.read(null)
      if (buf) {
        // - Send to the parser so that it parses. When it parses it emits the on('packet') event
  ...
  // - Handles data from the socket, when it is available
...
Client.prototype.publish = function (message, done) {
  console.log('--- Internal publish via Client ---', message)
